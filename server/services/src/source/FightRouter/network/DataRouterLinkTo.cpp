...
void DataRouterLinkTo::init_protocolhead()
{
	...
	s_head_.from_type_ = (S_INT_8)NETSERVICE_TYPE::ERK_SERVICE_FIGHTROUTER;
...
void DataRouterLinkTo::connect()
{
	...
	logInfo(out_runtime, "me(FightRouter) try to connect to DataRouter(iid:%ld ip:%s port:%d)",
...
void DataRouterLinkTo::on_cant_connectedto()
{
	...
	logInfo(out_runtime, "------me(FightRouter) cant connect to DataRouter(iid:%ld ip:%s port:%d)------",
		node_->iid, node_->ip.c_str(), node_->port);
...
void DataRouterLinkTo::on_connectedto_done()
{
	...
	logInfo(out_runtime, "++++++me(FightRouter) connected to DataRouter(iid:%ld ip:%s port:%d)++++++",
		node_->iid, node_->ip.c_str(), node_->port);
...
void DataRouterLinkTo::on_connected( bool success)
{
    if( success)
    {
		...
		req->set_svr_type(NETSERVICE_TYPE::ERK_SERVICE_FIGHTROUTER);
    ...
    {
		logError(out_runtime, "me(FightRouter) can't connect to DataRouter[ip:%s port:%d]", node_->ip.c_str(), node_->port);
...
void DataRouterLinkTo::on_authed( bool success)
{
    if( success)
    {
		logInfo(out_runtime, "me(FightRouter) connected to DataRouter[ip:%s port:%d]", node_->ip.c_str(), node_->port);
    ...
    {
		logInfo(out_runtime, "me(FightRouter) connect to DataRouter[ip:%s port:%d] failed", node_->ip.c_str(), node_->port);
    }
...
void DataRouterLinkTo::on_disconnected()
{
    ...
    if( this->is_authed())
    {
		logInfo(out_runtime, "me(FightRouter) disconnect from DataRouter[ip:%s port:%d]", node_->ip.c_str(), node_->port);
    }
