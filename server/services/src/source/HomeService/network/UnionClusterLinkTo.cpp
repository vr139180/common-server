...
void UnionClusterLinkTo::connect()
{
	...
	logInfo( out_runtime, "me(HomeService) try to connect to UnionService(iid:%ld ip:%s port:%d)", 
...
void UnionClusterLinkTo::on_cant_connectedto()
{
	...
	logInfo(out_runtime, "------me(GateService) cant connect to Union Service(iid:%ld ip:%s port:%d)------",
		node_->iid, node_->ip.c_str(), node_->port);
...
void UnionClusterLinkTo::on_connectedto_done()
{
	...
	logInfo(out_runtime, "++++++me(GateService) connected to Union Service(iid:%ld ip:%s port:%d)++++++",
		node_->iid, node_->ip.c_str(), node_->port);
...
void UnionClusterLinkTo::on_connected( bool success)
{
    ...
    {
        logError( out_runtime, "me(HomeService) can't connect to UnionService[ip:%s port:%d]", node_->ip.c_str(), node_->port);
        this->force_close();
    }
}
...
void UnionClusterLinkTo::on_authed( bool success)
{
    if( success)
    {
        logInfo( out_runtime, "me(HomeService) connected to UnionService[ip:%s port:%d]", node_->ip.c_str(), node_->port);
    ...
    {
        logInfo( out_runtime, "me(HomeService) connect to UnionService[ip:%s port:%d] failed", node_->ip.c_str(), node_->port);
        this->force_close();
...
void UnionClusterLinkTo::on_disconnected()
{
    ...
    if( this->is_authed())
    {
        logInfo( out_runtime, "me(HomeService) disconnect from UnionService[ip:%s port:%d]", node_->ip.c_str(), node_->port);
