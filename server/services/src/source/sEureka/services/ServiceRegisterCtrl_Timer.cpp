...
void ServiceRegisterCtrl::init_timer()
{
	...
	svrApp.add_apptimer(1000 * 5, boost::BOOST_BIND(&ServiceRegisterCtrl::gatebindhome_authtimeout_timer, this,
		boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3, boost::placeholders::_4));
}
...
void ServiceRegisterCtrl::gatebindhome_authtimeout_timer(u64 tnow, int interval, u64 iid, bool& finish)
{
	RedisClient* redis = svrApp.get_redisclient();

	//5 m
	static const S_INT_32 maintnce_step = 5;
	DTaskRedis dtask(GLOBAL_DTASK_SVRMAINTNCE_GATEHOMEAUTH, svrApp.get_eurekactrl()->get_myself().iid, maintnce_step);
	std::string taskkey = redis->build_rediskey(GLOBAL_DTASK_PREFIX, GLOBAL_DTASK_SVRMAINTNCE_GATEHOMEAUTH);
	...
	if (bexit)
	{
		...
		//结束当前定时器，调整下一个任务时间
		S_INT_32 timer = dtask.next_task_timer(2);
		if (timer == 0)
			return;

		svrApp.add_apptimer(timer, boost::BOOST_BIND(&ServiceRegisterCtrl::gatebindhome_authtimeout_timer, this,
			boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3, boost::placeholders::_4));

		return;
	}

	this->maintnce_gatebindhome_authtimeout(tnow);
}