...
#ifndef __LINKTOCONSISTENTHASH_H__
#define __LINKTOCONSISTENTHASH_H__
...
template<typename T, int VNODE_NUM = 800>
class LinkToConsistentHash
{
public:
	LinkToConsistentHash();
	virtual ~LinkToConsistentHash();
...
template<typename T, int VNODE_NUM>
LinkToConsistentHash<T, VNODE_NUM>::LinkToConsistentHash():cur_online_link_(0)
{
}

template<typename T, int VNODE_NUM>
LinkToConsistentHash<T, VNODE_NUM>::~LinkToConsistentHash()
{
	release();
}

template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::release()
{
	services_iid.clear();
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::free_all()
{
	release();
}

template<typename T, int VNODE_NUM>
bool LinkToConsistentHash<T, VNODE_NUM>::is_service_exist(S_INT_64 sid)
{
...
template<typename T, int VNODE_NUM>
bool LinkToConsistentHash<T, VNODE_NUM>::is_service_process(S_INT_64 sid)
{
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::connect_to()
{
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::send_mth_protocol(PRO::ERK_SERVICETYPE to, NetProtocol* pro)
{
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::send_mth_protocol(PRO::ERK_SERVICETYPE to, BasicProtocol* pro)
{
	size_t num = online_links_.size();
	if (num == 0)
	{
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::on_linkto_disconnected(T* plink)
{
	ThreadLockWrapper guard(lock_);
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::on_linkto_regist_result( T* plink)
{
	ThreadLockWrapper guard(lock_);
...
template<typename T, int VNODE_NUM>
void LinkToConsistentHash<T, VNODE_NUM>::sync_eureka_services(std::list<ServiceNodeInfo*>& nodes, std::list<S_INT_64>& deliids)
{
	ThreadLockWrapper guard(lock_);
...
#endif //__LINKTOCONSISTENTHASH_H__