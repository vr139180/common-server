...
{
	logInfo(out_sys, "game service[%lld] registed to eureka, success............", sid);
	this->is_ready_ = true;
}
...
void GameServiceApp::mth_eureka_losted()
{
	...
	logError(out_sys, "game service[%lld] lost all connections of eureka, service will shutdown......", EurekaClusterClient::instance().get_myiid());
	this->quit_app();
}
