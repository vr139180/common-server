...
bool GameServiceApp::load_config()
{
	...
	{
		logFatal(out_boot, "GameService load svr config file failed");
		return false;
	}
	...
	if (cf == 0)
	{
		logFatal(out_boot, "GameService load config file failed");
		return false;
	}
...
bool GameServiceApp::init_network()
{
	...
	{
		logFatal( out_boot, ("GameService init network failed"));
		return false;
	}
...
bool GameServiceApp::init_finish()
{
	...
	{
		logInfo(out_boot, ("GameService listen socket at %s:%d \n"), cf.get_ip().c_str(), cf.get_port());
	}
	else
	{
		logFatal(out_boot, ("GameService listen socket at %s:%d failed\n"), cf.get_ip().c_str(), cf.get_port());
		return false;
	}
...
void GameServiceApp::accept_netsession( NetAcceptorEvent::NetSessionPtr session, bool refuse, int err)
{
	...
	if (refuse)
	{
		logError(out_boot, "me(GameService) listen a connected request, but refused by system");
	...
	else
	{
		...
		logInfo(out_net, "me(GameService) listen a connected request, and create a connection successfully");
	}
}
...
void GameServiceApp::on_connection_timeout(GameSession* session)
{
	...
	logError(out_boot, "GameService listen a connected request, but this connection don't finish auth in a request time. system cut connection by self");
}
