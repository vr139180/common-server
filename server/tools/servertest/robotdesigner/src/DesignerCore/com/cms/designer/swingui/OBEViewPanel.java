	...
	implements ComponentListener, GraphModelListener, Observer
{
	...
	protected OBEGraphWorkflow graph;
	protected OBEGraphWorkflow originalGraph;
	protected PannerViewfinder v;
	protected Rectangle r;
	double graphWindowToPannerScale = 0.5;

	public OBEViewPanel(OBEDesigner parent, JViewport viewport, OBEGraphWorkflow g)
	{
		this.parent = parent;
		originalGraph = g;
		OBEViewPanel.viewport = viewport;
		
		init(g);
	}

	public void init( OBEGraphWorkflow g)
	{
		v = new PannerViewfinder(this, viewport);
		GraphLayoutCache view = new ViewRedirector(g, g.getGraphLayoutCache());
		graph = new OBEGraphWorkflow(parent, g.getOBEGraphModel(), view);
	...
	public static JPanel createOverviewPanel(OBEDesigner parent, OBEGraphWorkflow g)
	{
		JPanel panelWithInternalOffset = new JPanel();
		panelWithInternalOffset.setLayout(new BorderLayout());
		panelWithInternalOffset.setBorder(
			new MatteBorder(
				PANEL_BUFFER,
				PANEL_BUFFER,
				PANEL_BUFFER,
				PANEL_BUFFER,
				Color.WHITE));
		panelWithInternalOffset.add(
			new OBEViewPanel(parent, viewport, g),
			BorderLayout.CENTER);
	...
	public void componentResized(ComponentEvent e)
	{
		...
		Component componet = v.getViewport().getView();
		if (componet == null)
			return;
			...
			ChangeListener
	{
		...
		public PannerViewfinder(Container c, JViewport jvp)
		{
			...
			pannerResizeDecoration =
				new Rectangle(DECORATION_SIZE, DECORATION_SIZE);
		}
		...
		public void mousePressed(MouseEvent e)
		{
			...
			else
			{
				...
				pannerContainerRectangleCopy =
					new Rectangle(pannerContainerRectangle);
			}
		...
		private void SECornerResize(MouseEvent e)
		{
			...
			//recalculate scale
			double x_scale =
				(pannerViewRectangle.getWidth()
					/ pannerViewRectangleCopy.getWidth());
		...
		private int findClosestDragPoint(MouseEvent e, double buffer)
		{
			...
			int mask = 0;
			//leftVertical
			if (Math.abs(mx - lx) < buffer
				&& (my + buffer > uy && my < ly + buffer))
			{
				//lv
				mask |= LEFT_VERTICAL;
			}
			if (Math.abs(mx - rx) < buffer
				&& (my + buffer > uy && my < ly + buffer))
			{
				//rv
				mask |= RIGHT_VERTICAL;
			}
			if (Math.abs(my - uy) < buffer
				&& (mx + buffer > lx && mx < rx + buffer))
			{
				//uh
				mask |= UPPER_HORIZONTAL;
			}
			if (Math.abs(my - ly) < buffer
				&& (mx + buffer > lx && mx < rx + buffer))
			{
				//lh
		...
		{
			pannerViewRectangle.setLocation(
				last_x + e.getX(),
				last_y + e.getY());
		...
		public void update(Graphics g)
		{
			...
			pannerResizeDecoration.setLocation(
				maxx - DECORATION_SIZE / 2,
		...
		private void ensureWithinPannerBounds()
		{
			...
			if ((pannerViewRectangle.x + scaledWidth)
				> pannerContainerRectangle.getWidth())
			{
			...
			if ((pannerViewRectangle.y + scaledHeight)
				> pannerContainerRectangle.getHeight())
			{
				new_y =
					(int)pannerContainerRectangle.getHeight() - scaledHeight;
			}
		...
		private void ensureMaxResize()
		{
			...
			{
				Rectangle r =
					pannerViewRectangle.intersection(
						pannerContainerRectangleCopy);
		...
		private void ensureMinResize()
		{
			...
			if (pvrc.width > pvrc.height
				&& pannerViewRectangle.width < minSize)
			{
				pannerViewRectangle.width = minSize;
				pannerViewRectangle.height =
					(int) (pvrc.getHeight() * minSize / pvrc.getWidth());
			}
			else if (
				pvrc.height > pvrc.width
