...
// 协议解析和导出
type IProtobufFactory interface {
	// 协议对象转协议iid
	ProtoToId(proto proto.Message) (uint16, error)
	// 从id构造协议对象
	IdToProto(id uint16) (proto.Message, error)
}

// gnet使用的protobuf codec
type ProtobufCodec struct {
	// 协议工厂
	Factory IProtobufFactory
}

// 构造一个codec
func NewProtobufCodec(fact IProtobufFactory) (codec *ProtobufCodec, err error) {
	if fact == nil {
		return nil, errors.New("function NewProtobufCodec's fact parameter can't be set nil")
	}
...
func (cc *ProtobufCodec) Encode(c Conn, buf []byte) ([]byte, error) {
	return buf, nil
}

// Decode ...
func (cc *ProtobufCodec) Decode(c Conn) ([]byte, error) {
	// 检测是否符合协议流定义
	head := c.Read()
	buf := head[0:]
	totle := len(buf)
	...
	for {
		...
		// 单个协议长度不可能超过16k
		if len > 16*1024 {
			panic("ProtobufCodec - tcp steam error")
		}

		// 剩余的不是一个可用长度了
		if (totlen + onelen) > totle {
			break
		}
